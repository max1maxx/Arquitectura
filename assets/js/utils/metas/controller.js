...
function actualizarTabla(pagina, registros, inicio, fin) {
  var tablaBody = $("#tabla_metas tbody");
  tablaBody.empty();
...
function cargarDatosEnTabla(pagina, filtro) {
  $.post(
    "../controllers/Utils/metas.php?arqui=listar",
...
function cargarDatosInactivosEnTabla(pagina, filtro) {
  $.post(
    "../controllers/Utils/metas.php?arqui=listar_inactivos",
    { pagina: pagina, filtro: filtro },
    function (data) {
      datosTabla(pagina, filtro, data);
    }
...
function irAPagina(destino) {
  ...
  if (checkbox1.is(":checked")) {
    cargarDatosInactivosEnTabla(
      paginaActual,
      $("#filtro_metas").val().toLowerCase()
    );
  } else {
    cargarDatosEnTabla(paginaActual, $("#filtro_metas").val().toLowerCase());
  }
}
...
$(document).ready(function () {
  ...
  $("#filtro_metas").on("input", function () {
    paginaActual = 1;
  ...
  checkbox1.change(function () {
    ...
    var subtitulo_metas = document.getElementById("subtitulo_metas");
    paginaActual = 1;

    if (checkbox1.is(":checked")) {
      subtitulo_metas.textContent = "Lista de metas inactivos";
      // Checkbox activado
      cargarDatosInactivosEnTabla(paginaActual, "");

      checkbox2.prop("checked", true);
    } else {
      subtitulo_metas.textContent = "Lista de metas";
 ...
 */
// Activo
function eliminarActivo(id_met) {
  var id_tabla = "id_met";
  ...
  var url_ajax = "../controllers/Utils/metas.php?arqui=";
...
// Inactivo
function eliminarInactivo(id_met) {
  var id_tabla = "id_met";
  var texto = "activarlo";
...
// Eliminar
function eliminar(id_met) {
  // Obtener los datos del usuario para la funcion
  $.post(
    "../controllers/Utils/metas.php?arqui=mostrar_informacion_metas",
    { id_met: id_met },
    function (data) {
      data = JSON.parse(data);
      if (data.id_estado == 1) {
        eliminarActivo(id_met);
      } else if (data.id_estado == 2) {
        eliminarInactivo(id_met);
      }
    }
  );
}
...
function nuevo() {
  ...
  $("#pop_title_metas").html("NUEVO REGISTRO");
 ...
 * @param {*} id_met
 */
function editar(id_met) {
  $.post(
    "../controllers/Utils/metas.php?arqui=mostrar_informacion_metas",
    { id_met: id_met },
    function (data) {
      data = JSON.parse(data);
      $("#id_met").val(data.id_met);
      $("#id_ind").val(data.id_ind).trigger("change");
      $("#estado_met").val(data.estado_met);
      $("#linea_base_met").val(data.linea_base_met);
      $("#comportamiento_met").val(data.comportamiento_met);
      $("#unidad_medida_met").val(data.unidad_medida_met);
      $("#sentido_medicion_met").val(data.sentido_medicion_met);
      $("#denominador_met").val(data.denominador_met);
      $("#primer_trimestre_met").val(data.primer_trimestre_met);
      $("#segundo_trimestre_met").val(data.segundo_trimestre_met);
      $("#tercer_trimestre_met").val(data.tercer_trimestre_met);

      // Agregar titulo al pop
      $("#pop_title_metas").html("EDITAR REGISTRO");
...
// Funcion init
function init() {
  $("#metas_form").on("submit", function (e) {
    nuevo_actualizar(e);
  });
...
function nuevo_actualizar(e) {
  ...
  // No existe un id_met, por lo que se inserta uno nuevo
  if (!id_met) {
...
function enviarFormulario() {
  var formData = new FormData($("#metas_form")[0]);

  $.ajax({
    url: "../controllers/Utils/metas.php?arqui=nuevo_actualizar",
    type: "POST",
    data: formData,
    contentType: false,
    processData: false,
 ...
 * Mostrar modal
 */
$(document).on("click", "#btnAbrirModal_metas", function () {
  nuevo();
});
 ...
 * Cerrar modal
 */
$(document).on("click", "#btnCerrarModal_metas", function () {
  cancelar();
